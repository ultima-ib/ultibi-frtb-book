...
This user guide's primary focus is a very specific case - **[Fundamental Review of the Trading Book's Standardised Approach](https://en.wikipedia.org/wiki/Fundamental_Review_of_the_Trading_Book)**. For a more general usecase refer to the general user guide(TBA). Here we will focus on FRTB only. In general **`ultibi`** is a python interface of **Ultima**, a service which allows you to define, compute and breakdown the key performance metrics of your business.
